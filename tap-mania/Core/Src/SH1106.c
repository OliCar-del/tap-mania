...
// Ethan
void sidewaysScrollInRange(char *string2write, int *highlighted_offset) {
  int len = strlen(string2write);
  int maxPrintable = 20; // We can print ~20 7x11 characters to screen
  if ((len - *highlighted_offset) > maxPrintable) {
...
void write_to_oled(char ***songData, int song_count, int song_selected,
                   int *highlighted_offset) {
  SH1106_Fill(SH1106_COLOR_BLACK);
  SH1106_DrawFilledRectangle(0, TEXT_OFFSET_Y, 128, LINE_HEIGHT - 1, 1);
  int colour = 1;

  for (int i = 0; i < song_count; i++) {
    char name[64], artist[64], line[64 + 4 + 64 + 1];

    // Check if this is the config menu option (index 0 with special marker)
    bool is_config_option =
        (i == 0 && strcmp(songData[2][i], "__CONFIG__") == 0);

    if (is_config_option) {
      // Display config option differently
      strncpy(name, songData[0][i], sizeof(name) - 1);
      name[sizeof(name) - 1] = '\0';
      strip_eol(name);

      // For config, just show the name without "by Artist"
      snprintf(line, sizeof(line), "%s", name);
    } else {
      // Normal song display
      strncpy(name, songData[0][i], sizeof(name) - 1);
      name[sizeof(name) - 1] = '\0';
      strncpy(artist, songData[1][i], sizeof(artist) - 1);
      artist[sizeof(artist) - 1] = '\0';
      strip_eol(name);
      strip_eol(artist);

      snprintf(line, sizeof(line), "%s by %s", name, artist);
    }

    // Handle horizontal scrolling for selected item
    if (i == song_selected) {
      sidewaysScrollInRange(line, highlighted_offset);
    }

    SH1106.CurrentX = 2;
    if (i == song_selected) {
      colour = 0; // Inverted color for selected item
      SH1106.CurrentX -= *highlighted_offset;
    }

    SH1106.CurrentY =
        TEXT_OFFSET_Y + (i - song_selected) * LINE_HEIGHT + LINE_TEXT_Y_OFFSET;
    SH1106_Puts(line, &Font_7x10, colour);
    colour = 1;
  }
...
// Ethans very own function
// Song Data includes Song Name and Artist
// void write_to_oled(char*** songData, int song_count, int song_selected, int*
// highlighted_offset)
// {
//     SH1106_Fill(SH1106_COLOR_BLACK);
//     SH1106_DrawFilledRectangle(0, TEXT_OFFSET_Y, 128, LINE_HEIGHT - 1, 1);
//     int colour = 1;
//     myprintf("song count = %d\r\n", song_count);
//     for (int i = 0; i < song_count; i++) {
//         char* song = strdup(songData[0][i]);
//         char* artist = strdup(songData[1][i]);
//         // When pulled from the sd file, the data has a new line char, lets
//         get rid of that. song[strlen(song) - 1] = '\0'; artist[strlen(artist)
//         - 1] = '\0';

//         strcat(song, " by ");
//         strcat(song, artist);

//         sidewaysScrollInRange(highlighted_offset, song);

//         SH1106.CurrentX = 2;
//         printf("Trying to write %s to OLED\n\r", song);
//         if (i == song_selected) { // The highlight mechanic
//             colour = 0;
//             SH1106.CurrentX -= *highlighted_offset;
//         };

//         SH1106.CurrentY = TEXT_OFFSET_Y + (i - song_selected) * LINE_HEIGHT +
//         LINE_TEXT_Y_OFFSET; SH1106_Puts(song, &Font_7x10, colour); colour =
//         1;
//     }
//     SH1106_UpdateScreen();
// }
...
void SH1106_DrawFilledRectangle(uint16_t x, uint16_t y, uint16_t w, uint16_t h,
  ...
  if (y < 0) {
    y = 0;
  } // Another Ethan Fix. When will this library pay off \_(x_X)_/
